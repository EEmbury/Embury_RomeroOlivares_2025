...
#### precip
weather$month_site = paste(weather$Month, weather$SiteID, sep="_")

weather_sum <- aggregate(weather$Ppt_mm_Tot, by=list(category=weather$month_site), FUN=sum)

library(dplyr)
library(tidyr)

weather_sum_2 <- weather_sum %>% separate(category, c('Month', 'SiteID'))

precip_sum_avg <- weather_sum_2 %>%
  group_by(Month) %>%
  summarise_at(vars(x), list(name = mean))

sum(precip_sum_avg$name)


### temp

weather_no_na <- na.omit(weather)
weather_avg <- aggregate(weather_no_na$Air_TempC_Avg, by=list(category=weather_no_na$month_site), FUN=mean)
...
weather_avg_2 <- weather_avg %>% separate(category, c('Month', 'SiteID'))
temp_avg <- weather_avg_2 %>%
  group_by(Month) %>%
  summarise_at(vars(x), list(name = mean))
